...
**Scorecard rendering (v3.43).** The card front is drawn as vector art straight into the PDF (`src/lib/cardVector.js`), not rendered from HTML: type stays sharp at any size and cannot shift with web font loading. Geometry is the template's own CSS converted at 96px/in; type is Helvetica, which is what the reference PDF actually used (its "Archivo Expanded" never resolved) and which is built into PDF. The arrow in the call to action is drawn, since Helvetica carries no arrow glyph. The average chip is drawn at 88% opacity with real PDF transparency, as the template's rgba(15,18,26,0.88) has it, falling back to a solid fill where graphics state is unavailable. Gaps that the browser resolved differently from the CSS (tile gap 0.107in, well-to-tiles 0.18in, and the chip pinned 0.62in above the well bottom so it clears the score plate) are measured from the reference card and held by a test. The two plate labels (INDUSTRY AVERAGE, COMPASS SCORE) are set in Space Mono, embedded into the PDF from `public/scorecard/SpaceMono-Regular.ttf`; if that file cannot be loaded they fall back to Helvetica rather than failing the card. The back page never varies, so it ships as fixed artwork at `public/scorecard/card-back.png` (1575x2175px, 5.25x7.25in at 300dpi, corners filled and ground extended into the bleed); a build missing it fails with a clear message rather than printing a blank page. The slide is still rendered from HTML via html2canvas.
