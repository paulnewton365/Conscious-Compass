...
**Scorecard rendering notes (v3.41).** html2canvas ignores CSS filters and paints inline highlight boxes over the line above, both of which showed up in the first printed card. The Antenna logo therefore ships pre-whitened (`antenna-logo-white.png`) rather than filtered, and the "conscious brands" highlight sits on its own line as an inline-block. The card's back page is identical for every brand, so it prefers fixed artwork: drop a 5.25x7.25in image (1575x2175px at 300dpi, bleed included) at `public/scorecard/card-back.png` and it is used as page 2; without it the HTML back page is rendered as before.
